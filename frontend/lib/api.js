...
async function processAgentResponseWithAssistant(workflowId, conversation) {
  try {
    ...
    if (workflowId === journeys.disputing.id) {
      body.maxAnswers = 3; // For disputing journey, we want to allow more options to be generated by the assistant
      body.paymentDocument = store.getState().MandateLedger.journeysStatus?.[workflowId]?.paymentDocument || null; // Pass payment document for disputing journey to provide more context to the assistant
    }
