...
# Import the ADK auditor agent  
from agents.roles.auditor_agent.agent import root_agent as auditor_root_agent
...
# Create the ADK Runner for the auditor agent (following shopping pattern)
auditor_runner = Runner(
    app_name="auditor_agent", 
    agent=auditor_root_agent,
    artifact_service=artifact_service,
    session_service=session_service,
    memory_service=memory_service,
    #credential_service=credential_service,
...
async def call_auditor_agent(message: str, session_id: str, user_id: str) -> Dict[str, Any]:
    """Call the ADK auditor agent using the Runner pattern (same as shopping agent)"""
    try:
        logger.info(f"Calling auditor agent with message: {message}")
        ...
        # Call the agent with the actual user message using the auditor_runner
        content_message = types.Content(role="user", parts=[types.Part.from_text(text=message)],)
        ...
        logger.info(f"Received {len(events)} events from auditor agent")
        
        # Extract response text from events (same pattern as shopping agent)
        response_text = ""
        for event in events:
            # Extract text from ADK Event objects properly
            if hasattr(event, 'content') and event.content:
        ...
        response_text = response_text.strip() or "Hello! I'm your auditor agent. I can help verify payments, check mandate history, and test ledger integrity."
        
        logger.info(f"Final auditor response text: {response_text[:200]}...")
        ...
        logger.error(f"Error calling auditor agent: {e}")
        return {
            "response": f"I encountered an error: {str(e)}",
            "session_id": session_id,
            "user_id": user_id,
...
# FastAPI Auditor Agent Endpoints (following shopping agent pattern)
@app.post("/api/v1/auditor/chat", response_model=AuditorResponse, tags=["Auditor Agent"])
async def auditor_chat(request: AuditorRequest) -> AuditorResponse:
    """
    Chat with the ADK auditor agent - main conversation interface
    Supports natural language requests like:
    - "Verify payment 69d1d21e280d5346a7aaf00e"  
    - "Show me history for mandate MANDATE_12345"
    - "Test if someone can delete payment PAY_67890"
    """
    try:
        ...
        return AuditorResponse(**result)
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error in auditor chat: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def start_auditor_session(user_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "user_id": user_id,
        "status": "started",
        "initial_response": result.get("response", "Hello! I can help verify payments, check mandate history, and test ledger integrity. What would you like to audit?")
    }

@app.get("/api/v1/auditor/session/{session_id}", tags=["Auditor Agent"])
async def get_auditor_session(session_id: str, user_id: str) -> Dict[str, Any]:
    """
    Get auditor session state and conversation history
    """
    try:
        logger.info(f"Getting auditor session app_name=auditor_agent")
        ...
        logger.info(f"Auditor session retrieved: {session is not None}")
        
        if not session:
            logger.warning(f"Auditor session not found")
            raise HTTPException(status_code=404, detail="Session not found")
        
        # Safely extract session data (same pattern as shopping agent)
        session_data = {}
        try:
            if hasattr(session, '__dict__'):
                for key, value in session.__dict__.items():
        ...
        logger.error(f"Error getting auditor session for user {str(e)}")
        logger.error(f"Exception type: {type(e)}")
        raise HTTPException(status_code=500, detail=f"Auditor session service error: {str(e)}")
...
async def read_root(request: Request):
    return {
        ...
        "description": "ADK shopping and auditor agents wrapped in FastAPI endpoints for NextJS integration",
        "endpoints": {
            "shopping_chat": "/api/v1/shopping/chat",
            "shopping_start_session": "/api/v1/shopping/start-session",
            "shopping_session_info": "/api/v1/shopping/session/{session_id}",
            "auditor_chat": "/api/v1/auditor/chat", 
            "auditor_start_session": "/api/v1/auditor/start-session",
            "auditor_session_info": "/api/v1/auditor/session/{session_id}",
