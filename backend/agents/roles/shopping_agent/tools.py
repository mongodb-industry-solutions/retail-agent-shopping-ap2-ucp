...
from agents.ap2.types.contact_picker import ContactAddress
from agents.ap2.types.mandate import CART_MANDATE_DATA_KEY
from agents.ap2.types.mandate import CartMandate
from agents.ap2.types.mandate import PAYMENT_MANDATE_DATA_KEY
from agents.ap2.types.mandate import PaymentMandate
from agents.ap2.types.mandate import PaymentMandateContents
from agents.ap2.types.payment_request import PaymentResponse
from agents.common import artifact_utils
...
async def update_cart(
  ...
  # Get the chosen CartMandate from state to sign it
  cart_mandates: list[CartMandate] = tool_context.state.get("cart_mandates", [])
  chosen_cart_mandate = None
  for cart in cart_mandates:
    if cart.contents.id == chosen_cart_id:
      chosen_cart_mandate = cart
      break

  if not chosen_cart_mandate:
    raise RuntimeError(f"Chosen CartMandate with ID {chosen_cart_id} not found in state.")

  # Sign the CartMandate to prove user selected this specific cart
  cart_signature = {
      "signature": f"0x_shopping_agent_cart_sig_{datetime.now(timezone.utc).isoformat()}",
  ...
  message = (
      ...
      .add_data("cart_signature", cart_signature)
      ...
      .add_data("user_id", user_id or None)
      .add_data("session_id", session_id or None)
      .build()
  )
...
async def initiate_payment(tool_context: ToolContext, debug_mode: bool = False):
  ...
  # Extract payment signature to send to merchant
  payment_signature = {
      "signature": payment_mandate.user_authorization,
      "signer_id": "trusted_shopping_agent",
      "signer_type": "shopping-agent",
      "algorithm": "UserDevice",
      "signed_at": datetime.now(timezone.utc).isoformat()
  }
  ...
  outgoing_message_builder = (
      ...
      .add_data("payment_signature", payment_signature)
